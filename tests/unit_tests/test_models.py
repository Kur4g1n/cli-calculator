...
def test_constant_node():
    ...
    # Without overflow
    assert ConstantNode(1e999).evaluate(OperatorSettings.NONE) == float("inf")
...
def test_constant_node_error():
    ...
    with pytest.raises(ExpressionOverflowError) as e_info:
        ConstantNode(num).evaluate(OperatorSettings.LIMIT_FLOATS)
...
def test_binary_node():
    assert (
        BinaryOperatorNode(
            binary_operator_registry[ast.Add], ConstantNode(1), ConstantNode(1)
        ).evaluate(OperatorSettings.LIMIT_FLOATS)
        == 2
    )
    assert BinaryOperatorNode(
        binary_operator_registry[ast.Sub], ConstantNode(20), ConstantNode(43.127)
    ).evaluate(OperatorSettings.LIMIT_FLOATS) == pytest.approx(-23.127, 1e-11)
    assert (
        BinaryOperatorNode(
            binary_operator_registry[ast.Mult], ConstantNode(4), ConstantNode(10)
        ).evaluate(OperatorSettings.LIMIT_FLOATS)
        == 40
    )
    assert BinaryOperatorNode(
        binary_operator_registry[ast.Div], ConstantNode(13), ConstantNode(7)
